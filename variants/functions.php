...
class Functions extends Database
{
  ...
  **/
  ...
  {
    $sql = "SELECT p.productname, p.image, v.size, v.price, v.id FROM {$this->tableName} as v INNER JOIN products as p ON v.productid=p.id WHERE v.productid=$id LIMIT {$start},{$limit}";
  ...
  public function getCount($id)
    {
        $sql = "SELECT count(*) as pcount FROM {$this->tableName} WHERE productid={$id}";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
