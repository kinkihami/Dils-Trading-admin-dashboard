...
if ($action == "addcategory" && !empty($_POST)) {
    try {
        ...
        $path = '../category_image/'; // upload directory
        ...
        if ($playerId) {
            if (!empty($_FILES["image"]['name'])) {
                ...
                if (!empty($category) && file_exists('../category_image/' . $category)) {
                    unlink('../category_image/' . $category);
                }
            ...
            if (!empty($_FILES["image"]['name'])) {
                file_put_contents($logFile, "IAM INSIDE THIS", FILE_APPEND | LOCK_EX);
...
if ($action == "getcategory") {
    ...
    file_put_contents($logFile, "inside get =catefory", FILE_APPEND | LOCK_EX);
    $page = (!empty($_GET['page'])) ? $_GET['page'] : 1;
    $limit = 15;
    $start = ($page - 1) * $limit;

    $category = $obj->getRows($start, $limit);
